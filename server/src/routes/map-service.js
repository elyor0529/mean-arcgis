...
function registerMapEndpoints(router) {
    ...
    registerDataFormatEndpoint(router, 'csv', 'text/csv', fetchAllDonors, (donorArray) => json2csv({
        data: donorArray,
        quotes: '',
        fields: [].concat(DONOR_REST_FIELDS, ['id'])
    }), {});

    registerDataFormatEndpoint(router, 'geojson', 'application/json', fetchAllDonors, (donorArray) => GeoJSON.parse(donorArray, {Point: ['latitude', 'longitude']}), {});
}
